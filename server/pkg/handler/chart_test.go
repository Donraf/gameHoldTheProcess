...
func TestHandler_deleteChart(t *testing.T) {
	type mockBehavior func(r *service.MockChart, id string)
	...
	tests := []struct {
	...
	}{
		{
			...
			mockBehavior: func(r *service.MockChart, id string) {
				idInt, _ := strconv.Atoi(id)
				r.EXPECT().DeleteChart(idInt).Return(nil)
			},
			expectedStatusCode:  200,
		...
		},
		{
			...
			mockBehavior: func(r *service.MockChart, id string) {
				idInt, _ := strconv.Atoi(id)
				r.EXPECT().DeleteChart(idInt).Return(errors.New(""))
			},
			expectedStatusCode: 500,
			isError:            true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gin.SetMode(gin.TestMode)
			r := gin.New()
			r.DELETE("/:id", handler.deleteChart)
