...
func (u *UserPostgres) GetParSet(id int) (gameServer.ParameterSet, error) {
	...
	query := fmt.Sprintf("SELECT id, gain_coef, time_const, noise_coef, false_warning_prob, missing_danger_prob  FROM %s AS ut JOIN %s AS pst ON ut.cur_par_set_id=pst.id WHERE user_id=$1", usersTable, parameterSetsTable)
...
func (u *UserPostgres) GetPlayersStat(input gameServer.GetPlayersStatInput) ([]gameServer.PlayerStat, error) {
	...
	for _, user := range users {
		...
		query = fmt.Sprintf("SELECT pst.id, pst.gain_coef, pst.time_const, pst.noise_coef, pst.false_warning_prob, pst.missing_danger_prob FROM %s AS pst JOIN %s AS upst ON pst.id=upst.parameter_set_id WHERE upst.user_id=$1", parameterSetsTable, userParameterSetsTable)
